...
namespace VideoPlatform.DAL.Interfaces
{
    public interface IMetaEntityRepository<TEntity> where TEntity : MetaEntity
    {
        ...
        Task<TEntity> GetMetaEntityAsync(Expression<Func<TEntity, bool>> filterExpression = null, CancellationToken cancellationToken = default);

        Task<ICollection<TEntity>> GetMetaEntitiesAsync(Expression<Func<TEntity, bool>> filterExpression = null, CancellationToken cancellationToken = default);

        Task<PagingResult<TEntity>> GetPagingMetaEntitiesAsync(MetaPaging<TEntity> pagingModel, CancellationToken cancellationToken = default);
        ...
        Task<IList<TEntity>> CreateMetaEntitiesAsync(IList<TEntity> entities, CancellationToken cancellationToken = default);
