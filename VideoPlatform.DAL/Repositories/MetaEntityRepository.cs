...
public abstract class MetaEntityRepository<TEntity> : IMetaEntityRepository<TEntity> where TEntity : MetaEntity
{
    ...
    public async Task<TEntity> GetMetaEntityAsync(Expression<Func<TEntity, bool>> filterExpression = null,
    ...
    public async Task<ICollection<TEntity>> GetMetaEntitiesAsync(
    ...
    public async Task<PagingResult<TEntity>> GetPagingMetaEntitiesAsync(MetaPaging<TEntity> pagingModel,
    ...
    public async Task<IList<TEntity>> CreateMetaEntitiesAsync(IList<TEntity> entities,
        CancellationToken cancellationToken)
    {
        using var session = await Client.StartSessionAsync(cancellationToken: cancellationToken);
        session.StartTransaction(
    ...
    public async Task<bool> UpdateMetaEntitiesAsync(IList<TEntity> entities, CancellationToken cancellationToken)
    {
        ...
        try
        {
            foreach (var entity in entities)
            {
                var dbEntity = await _collection.Find(x => x.Id.Equals(entity.Id))
