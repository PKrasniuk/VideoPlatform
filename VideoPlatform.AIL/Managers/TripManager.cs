...
namespace VideoPlatform.AIL.Managers
{
    public class TripManager : IManager<TripModel, TripFarePredictionModel, RegressionMetrics>
    {
        ...
        {
            if (!Directory.Exists(_dataSetsPath) || !File.Exists(TrainDataPath) || !File.Exists(TestDataPath))
            {
                return null;
            }
            ...
            var dataProcessPipeline = _mlContext.Transforms.CopyColumns("Label", nameof(TripModel.FareAmount))
                ...
                .Append(_mlContext.Transforms.Categorical.OneHotEncoding("PaymentTypeEncoded", nameof(TripModel.PaymentType)))
        ...
        public ICollection<TripFarePredictionModel> CalculatePrediction(IEnumerable<TripModel> items, bool rebuildModel = false)
        {
            ...
            var predictionEngine = _mlContext.Model.CreatePredictionEngine<TripModel, TripFarePredictionModel>(trainedModel);

            var result = new Collection<TripFarePredictionModel>();
            foreach (var item in items)
            {
                result.Add(predictionEngine.Predict(item));
            }
            return result;
        }
    }
